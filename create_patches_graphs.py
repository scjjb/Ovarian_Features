# internal imports
from wsi_core.WholeSlideImageGraph import WholeSlideImage
...
def patching(WSI_object, **kwargs):
        ...
        # Patch
        file_path_big, file_path_small = WSI_object.process_contours(**kwargs)


        ### Stop Patch Timer
        patch_time_elapsed = time.time() - start_time
        return file_path_big, file_path_small, patch_time_elapsed
...
def seg_and_patch(source, save_dir, patch_save_dir, mask_save_dir, stitch_save_dir, 
        ...
        for i in range(total):
                ...
                if auto_skip and os.path.isfile(os.path.join(patch_save_dir,"big", slide_id + '.h5')):
                ...
                if patch:
                    ...
                    file_path_big, file_path_small, patch_time_elapsed = patching(WSI_object = WSI_object,  **current_patch_params,)
                
                stitch_time_elapsed = -1
                if stitch:
                        file_path_big = os.path.join(patch_save_dir,"big", slide_id+'.h5')
                        if os.path.isfile(file_path_big):
                                heatmap, stitch_time_elapsed = stitching(file_path_big, WSI_object, downscale=64)
                                stitch_path_big = os.path.join(stitch_save_dir,"big", slide_id+'.jpg')
                                heatmap.save(stitch_path_big)
                        file_path_small = os.path.join(patch_save_dir,"small", slide_id+'.h5')
                        if os.path.isfile(file_path_small):
                                heatmap, stitch_time_elapsed = stitching(file_path_small, WSI_object, downscale=64)
                                stitch_path_small = os.path.join(stitch_save_dir,"small", slide_id+'.jpg')
                                heatmap.save(stitch_path_small)
...
if __name__ == '__main__':
        ...
        extra_graph_directories = {'patch_save_dir_big': patch_save_dir+"/big",
                                    'patch_save_dir_small': patch_save_dir+"/small",
                                    'stitch_save_dir_big': stitch_save_dir+"/big",
                                    'stitch_save_dir_small': stitch_save_dir+"/small"}
        for key, val in directories.items():
                print("{} : {}".format(key, val))
                if key not in ['source']:
                        os.makedirs(val, exist_ok=True)

        for key, val in extra_graph_directories.items():
                print("{} : {}".format(key, val))
                os.makedirs(val, exist_ok=True)
