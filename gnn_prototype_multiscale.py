...
parser.add_argument('--small_features_folder', type=str, default="graph_ovarian_leeds_resnet50imagenet_256patch_features_10x/pt_files",help='folder within data_root_dir containing the small patch (higher magnification) features - must contain pt_files/h5_files subfolder')
parser.add_argument('--coords_dir', type=str, default="../mount_outputs/patches/ovarian_leeds_mag40x_patchgraph2048and1024_fp/patches/big",help="directory containing the lower magnification (bigger base patches) coordinates")
parser.add_argument('--small_coords_dir', type=str, default="../mount_outputs/patches/ovarian_leeds_mag40x_patchgraph2048and1024_fp/patches/small",help="directory containing the higher magnification (small base patches) coordinates")
...
class GraphDataset(Dataset):
    def __init__(self, node_features_dir, small_node_features_dir, coordinates_dir, small_coordinates_dir, csv_path, max_nodes = 250, transform=None, pre_transform=None):
        #super(GraphDataset, self).__init__(root, transform, pre_transform)
        self.node_features_dir = node_features_dir
        self.small_node_features_dir = small_node_features_dir
        self.coordinates_dir = coordinates_dir
        self.small_coordinates_dir = small_coordinates_dir
    ...
    def process(self):
        ...
        for i in tqdm(range(total_slides)):
            ...
            small_node_features = torch.load(os.path.join(self.small_node_features_dir, str(slide_name)+".pt"))
            with h5py.File(os.path.join(self.small_coordinates_dir, str(slide_name)+".h5"),'r') as hdf5_file:
            ...
            if len(coordinates)+len(small_coordinates)>self.max_nodes_in_dataset:
                self.max_nodes_in_dataset=len(coordinates)+len(small_coordinates)
            
            distances = pdist(coordinates, 'euclidean')
            dist_matrix = squareform(distances)
            dist_threshold = 3000  # Adjust this threshold as needed
            adj = (dist_matrix <= dist_threshold).astype(int)
            adj = (adj - np.identity(adj.shape[0])).astype(int)
            ...
            dist_matrix = squareform(distances)
            dist_threshold = 3000/2  # Adjust this threshold as needed - taking it as half the above, which gives about 4x as many total edges
            adj = (dist_matrix <= dist_threshold).astype(int)
            adj = (adj - np.identity(adj.shape[0])).astype(int)
            edge_indices = np.transpose(np.triu(adj,k=1).nonzero())
            ...
            ## now need to connect parents to children
            ## first gonna find the distances from the small coordinates to each big coordinates, then can take the minimum
            
            ## all coordinates are in the corner of the image, so we need a slight adjustment to get the right assignments between small and big patches
            ## the adjustment should be half the size of the small patches 
            #print(cdist(small_coordinates,coordinates, 'euclidean'))
            distances = cdist(small_coordinates,coordinates+512, 'euclidean')
            #print(distances)
            
            ## after the adjustment, each small patch will be within one small patch distance of its parent and will not be as close to any other
            ## specifically the distance will be sqrt(adjustment^2 + adjustment^2)
            dist_threshold = 1024  # Adjust this threshold as needed - taking it as half the above, which gives about 4x as many total edges
            adj = (distances <= dist_threshold).astype(int)
            edge_indices = np.transpose(adj.nonzero())
            adj_parents = torch.from_numpy(edge_indices).t().contiguous()
            
            ## need to correct the small patches to have the right indices
            adj_parents[0] += coordinates.shape[0]
            #print(distances)
            #print(adj)

            #dist_matrix = squareform(distances)
            
            adj_all = torch.cat((adj_big, adj_small, adj_parents),dim=1)
            #print(adj_all)
            #print(adj_all.shape)
            #print(distances)
            #print(small_coordinates)
            #print(small_coordinates)
            #print(coordinates+512)
            ## now need to connect parents to children

            adj = adj_all
            x_big = node_features.clone().detach()
            x_small = small_node_features.clone().detach()
...
dataset = GraphDataset(node_features_dir=data_dir, small_node_features_dir=small_data_dir, coordinates_dir=args.coords_dir, small_coordinates_dir=args.small_coords_dir, csv_path = args.csv_path,max_nodes=args.max_nodes)
...
## fold 0 only
train_dataset, val_dataset, test_dataset = dataset.return_splits(csv_path='{}/splits_{}.csv'.format(args.split_dir, 0))
...
def train(epoch,weight):
    ...
    for data in train_loader:
        ...
        try:
            ...
            assert 1==2, "broken slide with data{}".format(data)
